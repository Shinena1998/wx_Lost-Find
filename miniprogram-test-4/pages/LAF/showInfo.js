...
Page({
  ...
  */
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        for(var i = 0 ; i < res.data.length ; i ++){
          ...
          console.log(res.data[i].look)
          if(res.data[i].look == false){
            res.data[i].pushInfo.confirm = true
          }else{
            res.data[i].pushInfo.confirm = false
          }
          that.data.category.push(res.data[i].pushInfo)
          console.log(res.data[i].pushInfo.confirm)
        }
        that.setData({
          category: that.data.category
        })
  ...
  showComment:function(index){
    ...
    if (index == 4) {
      this.data.comment = JSON.parse(JSON.stringify(app.globalData.comment.reply));
      for (var i = 0; i < this.data.comment.length; i++) {
        this.data.comment[i].commentInfo.toName = '@' + this.data.comment[i].commentInfo.toName + "："
      }
    } else if (index == 5) {
      this.data.comment = JSON.parse(JSON.stringify(app.globalData.comment.info));
      for (var i = 0; i < this.data.comment.length; i++) {
